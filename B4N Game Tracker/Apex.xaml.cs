...
namespace B4N_Game_Tracker
{
    ...
    /// </summary>
    public sealed partial class Apex : Page
    {
            ...
            else
            {
                ...
                else if (database == "Local")// If local database is chosen. Using a database we got to use in .NET lecture.
                {
                    try
                    {
                        ...
                        SqlCommand cmdUserName = new SqlCommand("SELECT UserName FROM dbo.GametrackerApex WHERE UserName ='" + username + "' AND Platform ='" + platform + "'", conn);
                        SqlCommand cmdKills = new SqlCommand("SELECT Kills FROM dbo.GametrackerApex WHERE UserName ='" + username + "' AND Platform ='" + platform + "'", conn);
                        SqlCommand cmdDeath = new SqlCommand("SELECT Death FROM dbo.GametrackerApex WHERE UserName ='" + username + "' AND Platform ='" + platform + "'", conn);
                        SqlCommand cmdLastCharachter = new SqlCommand("SELECT LastCharacter FROM dbo.GametrackerApex WHERE UserName ='" + username + "' AND Platform ='" + platform + "'", conn);
